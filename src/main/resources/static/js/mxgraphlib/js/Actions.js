...
Actions.prototype.init = function()
{
	...
	function deleteCells(includeEdges)
	{
		...
		{
			if(graph.getSelectionCell().value != '' && graph.getSelectionCell().value != null && graph.getSelectionCell().value.attributes != null && graph.getSelectionCell().value.attributes[0].nodeValue === "Start")
			{
				Swal.fire({
					title: 'Delete Error',
					text:  "We can't delete the Start operator",
					type:  'error',
					timer: 2000,
					showConfirmButton: false
				});
			}
			else if (graph.getSelectionCell().value != '' && graph.getSelectionCell().value != null && graph.getSelectionCell().value.attributes != null &&  graph.getSelectionCell().value.attributes[0].nodeValue === "End")
			{
				Swal.fire({
					title: 'Delete Error',
					text:  "We can't delete the End operator",
					type:  'error',
					timer: 2000,
					showConfirmButton: false
				});
