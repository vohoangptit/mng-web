...
//	this.diagramContainer.style.left = (this.hsplit.parentNode != null) ? (effHsplitPosition + this.splitSize) + 'px' : '0px';
	...
	console.log(this.sidebarContainer.style.top);
...
EditorUi.prototype.createUi = function()
{
	...
	buttonCancel.style.margin = 2+'px';
	this.editor.graph;
	mxEvent.addListener(buttonCancel, 'click', function(evt)
			{
				disableLeavePageAlert();
				var wfId = $('#wfId').val();
				window.top.location.href = '/menu/workflow/workflow-history/'+wfId;
			});
	...
	// 	callDeleteApi($('#workflowid', parent.document).val())
	// });
	//
    ...
    //             workflowOperator['contentXML'] = xml.outerHTML;
    //             if(xml.outerHTML.indexOf("Start") > -1) {
    ...
    //                 }
    //             } else if(xml.outerHTML.indexOf("ellipse") > -1 && cell.value.attributes[0].nodeValue != "Start" && cell.value.attributes[0].nodeValue != "End"){
    //                 if(cell.value.attributes[0].nodeValue == '') {
    //                     checkDataEmpty = false;
	...
	// 					$.post(url,{'format':'png','filename':workflowname,'bg':'#ffffff','w':w,'h':h,'xml':param} ,function( data ) {
	// 						console.log(data);
	// 					});
	...
	// buttonExport.style.margin = 2+'px';
	//
	...
	//
	// 		});
	// mxUtils.write(buttonExport, 'Export Image');

	var buttonRestore = this.createDiv('btnHistory');
	buttonRestore.setAttribute("class", "btn btn-light");
	buttonRestore.style.margin = 2+'px';
	mxEvent.addListener(buttonRestore, 'click', function(evt)
	{
		var historyId = $('#workflowid', parent.document).val();
		$.ajax({
			type: 'GET',
	...
	mxUtils.write(buttonRestore, 'Restore');
	...
	this.contentColNameToolbar = this.createDiv('col-md-3');
	this.contentColNameToolbar.appendChild(spanName);
	this.contentColNameToolbar.appendChild(inName);

	this.contentColDescriptionToolbar = this.createDiv('col-md-7');
	...
	this.contentButtonNeraToolbar.appendChild(buttonRestore);
