...
var EditorUi = function (editor, container, lightbox) {
	...
	for (var iStyle = 0; iStyle < connectStyles.length; iStyle++) {
		if (mxUtils.indexOf(styles, connectStyles[iStyle]) < 0) {
			styles.push(connectStyles[iStyle]);
		}
	}
...
EditorUi.prototype.isSelectionAllowed = function (evt) {
	return mxEvent.getSource(evt).nodeName == 'SELECT' || (mxEvent.getSource(evt).nodeName == 'INPUT' &&
		mxUtils.isAncestorNode(this.formatContainer, mxEvent.getSource(evt))) || (mxEvent.getSource(evt).nodeName == 'TEXTAREA' &&
...
//	this.sidebarContainer.style.top = tmp/2 + 'px';
	this.sidebarContainer.style.top = 47 + 'px';
//	this.sidebarContainer.style.width = effHsplitPosition + 'px';
	this.sidebarContainer.style.width = 100 + '%';
//	this.formatContainer.style.top = tmp/2 + 'px';
	this.formatContainer.style.top = 100 + 'px';
...
//	this.diagramContainer.style.left = (this.hsplit.parentNode != null) ? (effHsplitPosition + this.splitSize) + 'px' : '0px';
	...
	this.diagramContainer.style.top = 100 + 'px';
...
EditorUi.prototype.createDivs = function () {
	this.neraToolbar = this.createDiv('geNeraToolbar');
//	this.neraToolbar.style.width = 30+'px';
	this.menubarContainer = this.createDiv('geMenubarContainer');
	this.sidebarContainer = this.createDiv('geSidebarContainer');
	this.formatContainer = this.createDiv('geSidebarContainer geFormatContainer');
	// this.formatContainer = this.createDiv('geSidebarContainer abc');
	...
	this.menubarContainer.id = 'menucontainer';
...
EditorUi.prototype.createUi = function () {
	...
	ui = this;
	this.contentRowNeraToolbar = this.createDiv('row');
	this.contentTextfieldNeraToolbar = this.createDiv('btncontaint responsiveHeaderWorkflow');
	this.contentTextfieldNeraToolbar.style.paddingTop = '7px';

	this.contentButtonNeraToolbar = this.createDiv('btncontaint');
	this.contentButtonNeraToolbar.style.textAlign = 'right';
	// this.contentNeraToolbar.style.
	var buttonCancel = document.createElement('button');
	buttonCancel.setAttribute("class", "btn btn-light");
	buttonCancel.style.margin = 1 + 'px';
	var graph = this.editor.graph;
	mxEvent.addListener(buttonCancel, 'click', function (evt) {
		disableLeavePageAlert();
		$('#cancelWorkflow').modal('show');
		// var isSave = $('#isSave', parent.document).val();

		// window.top.location.href = '../menu/workflow/workflow-listing';
		// if (isSave === '0') {
	...
	$('#btnLeavePageWorkflow').on('click', function () {
		disableLeavePageAlert();
		window.top.location.href = '../menu/workflow/workflow-listing';
	});
	...
	buttonDel.setAttribute("class", "btn btn-light");
	if (!hasDeleteWorkflowPermission || $('#workflowid', parent.document).val() == '') {
		buttonDel.disabled = true;
	}
	buttonDel.style.margin = 1 + 'px';
	mxEvent.addListener(buttonDel, 'click', function (evt) {
		$('#deleteWorkflowConfirmation').modal('show');
	});
	mxUtils.write(buttonDel, 'Delete');
	var buttonSave = this.createDiv('btnsave');
	buttonSave.setAttribute("class", "btn btn-light");
	buttonSave.style.margin = 1 + 'px';

	$('#btnYesDeleteWorkflow').on('click', function () {
	...
	mxEvent.addListener(buttonSave, 'click', function (evt) {
		var isError = $('#isError', parent.document).val();
		var messageError = $('#messageError', parent.document).val();
		if (isError == '1') {
			Swal.fire({
				title: 'Save Error',
				text: messageError,
			...
			$('#saveWorkflowConfirmation').modal();
		}
	});

	$('#btnYesSaveWorkflow').on('click', function () {
		$("#saveWorkflowConfirmation").modal('hide');
		var json = {};
		var enc = new mxCodec();
		// var cells = graph.cloneCells(graph.getModel().root.getChildAt(0).children);
		var cells = graph.getModel().root.getChildAt(0).children;
		var workflowname = $('#WorkflowName').val();
		if (workflowname != null && workflowname != '') {
			json['name'] = $('#WorkflowName').val();
			json['isActive'] = true;
			json['del'] = false;
			json['description'] = $('#DescriptionName').val();
			var workflowOperators = [];
			var checkDataEmpty = true;
			var messageError = '';
			$.each(cells, function (i, cell) {
				...
				var start = 0;
				var xml = enc.encode(cell);
				var workflowOperator = {};
				var contentXML;
				if (typeof xml.outerHTML !== 'undefined') {
					contentXML = xml.outerHTML;
				} else {
					contentXML = new XMLSerializer().serializeToString(xml);
				}

				if (contentXML != null) {
					if (contentXML.indexOf('xmlns="http://www.w3.org/1999/xhtml') > -1) {
						contentXML = contentXML.replace('xmlns="http://www.w3.org/1999/xhtml"', '');
					}
					if (contentXML.indexOf('Start') > -1) {
						if (cell.getEdgeCount() > 1) {
							checkDataEmpty = false;
							messageError = 'Only 1 connector can go outward the Start.';
						} else if (cell.getEdgeAt(0) != null && cell.getEdgeAt(0).target.getAttribute('label') === 'Start') {
							checkDataEmpty = false;
							messageError = 'Connector can not go inward the Start.';
						}
					}  else if (contentXML.indexOf('rhombus') > -1) {
						if (cell.value.attributes[0].nodeValue == '') {
							checkDataEmpty = false;
							messageError = 'Please input switch case name.';
						} else {
							var index = 1;
							while(typeof cell.value.getAttribute('value'+start+'_'+index) !== 'undefined' && cell.value.getAttribute('value'+start+'_'+index) !== null) {
								if(cell.value.getAttribute('value'+start+'_'+index) == '') {
									checkDataEmpty = false;
									messageError = 'Please input value for condition ' + cell.value.getAttribute('paramkey'+start+'_'+index);
									break;
								}
								index++;
							}
						}
					} else if (contentXML.indexOf('End') > -1) {
						if (cell.getEdgeAt(0) != null && cell.getEdgeAt(0).source.getAttribute('label') === 'End') {
							checkDataEmpty = false;
							messageError = 'Connector can not go outward the End.';
						}
					} else if (contentXML.indexOf('Playbook') > -1) {
						for (var n = 0; n < cell.value.childNodes[0].querySelectorAll('PLaybookInputElement').length; n++) {
							if (cell.value.childNodes[0].querySelectorAll('PLaybookInputElement')[n].getAttribute('manda') === 'true' && cell.value.childNodes[0].querySelectorAll('PLaybookInputElement')[n].getAttribute('value_' + n) === '') {
								checkDataEmpty = false;
								messageError = 'Please make assignment to mandatory input.';
							}
						}
					} else if (contentXML.indexOf('triangle') > -1) {
						if (cell.value == null || cell.value == '' || cell.value.attributes[0].nodeValue == '' || cell.value.attributes[1].nodeValue == '') {
							checkDataEmpty = false;
							messageError = 'Please select an Approver';
						}
					} else if (contentXML.indexOf('ellipse') > -1 && cell.value.attributes[0].nodeValue != 'Start' && cell.value.attributes[0].nodeValue != 'End') {
						if (cell.value.attributes[0].nodeValue == '') {
							checkDataEmpty = false;
							messageError = 'Please input status name';
						} else if (cell.value.attributes[1].nodeValue == '') {
							checkDataEmpty = false;
							messageError = 'Please input the status';
						}
					}
				}
				workflowOperator['contentXML'] = contentXML;

				// else if(xml.outerHTML.indexOf("Playbook") > -1 && xml.outerHTML.indexOf('manda="true"') == -1) {
				// 	checkDataEmpty = false;
				// 	messageError = 'Please make assignment to mandatory input';
				// }
				if (checkDataEmpty && cell.value != null) {
					workflowOperator['properties'] = cell.value.attributes[0].nodeValue;
				}
				// workflowOperator['type'] = graph.view.getState(cell).style.shape;
				workflowOperators.push(workflowOperator);
				start++;
			});
			if (!checkDataEmpty) {
				Swal.fire({
			...
			json['workflowOperator'] = workflowOperators;
			//
			for (var i = 0; i < dataJob.length; i++) {
				delete dataJob[i].idJobList
			}
			json['jobInput'] = dataJob;
			var workflowId = $('#workflowid', parent.document).val();
			var cloneId = $('#cloneId', parent.document).val();
			var urlWorkflow = '/nera/api/save-workflow';
			if (cloneId === '' && workflowId > 0) {
				json['id'] = workflowId;
				json['version'] = parseInt($('#txtVersion').val()) + 1;
				urlWorkflow = '/nera/api/update-workflow';
			}
			$.ajax({
				type: 'POST',
				url: urlWorkflow,
				...
				success: function (data) {
					...
					if (data.code == '200') {
						...
						xmlCanvas.translate(Math.floor((0 / 1 - bounds.x) / graph.view.scale),
							Math.floor((0 / 1 - bounds.y) / graph.view.scale));
						xmlCanvas.scale(1 / graph.view.scale);
						...
						var w = Math.ceil(bounds.width * 1 / graph.view.scale + 2 * 0);
						var h = Math.ceil(bounds.height * 1 / graph.view.scale + 2 * 0);

						var url = '/nera/api/create-image';
						$.post(url, {
							'format': 'png',
						...
						Cookies.set('workflowName', workflowname);
						window.top.location.href = '/menu/workflow/workflow-listing';
	...
	mxUtils.write(buttonSave, 'Save');
	var buttonJob = this.createDiv('btnJob');
	buttonJob.setAttribute("class", "btn btn-light");
	buttonJob.style.margin = 1 + 'px';

	var dataJob = [];

	loadDataJobDB();

	function loadDataJobDB() {
		var idWorkflow = $('#workflowid', parent.document).val();
		if (idWorkflow == null || idWorkflow == '') {
			return;
		}
		$.ajax({
			type: 'POST',
			data: {id: idWorkflow},
			url: '/nera/api/get-job-by-id',
			success: function (data) {
	...
	mxEvent.addListener(buttonJob, 'click', function (evt) {
		// loadDataJob();
		$('#jobinputworkflow').modal("show");
	});
	mxUtils.write(buttonJob, 'Job input');


	function loadDataJob() {
		// $('#tblPlaybookInput2 > tbody > tr:nth-child(n+1)').empty();
		var tableLength = $('#tblPlaybookInput2')[0].rows.length;

		for (var n = tableLength; n >= 3; n--) {
			$('#tblPlaybookInput2')[0].deleteRow(n - 1);
		}

		for (var i = 0; i < dataJob.length; i++) {
			appendNewRow(dataJob[i].type, dataJob[i].variable, dataJob[i].idJobList);
		}
		$('#selectedType').val('1');
		$('#selectedType').select2().trigger('change');
	...
	// 	$('#tblPlaybookInput').removeClass('hideclass');
	// 	if($('#tblAddInput').attr('class') == 'standard hideclass') {
	// 		$('#tblAddInput').removeClass('hideclass');
	// 	} else {
	// 		$('#tblAddInput').addClass('hideclass');
	// 	}
	// });

	function pldAddNewRow() {
		var colDataIdType = $('#selectedType option:selected').val();
		var colVariable = '';
		var colDataID = '';
		var idJobData = $('#txtIdJobInput').val();
		var indexJob = -1;
		var idJob = 0;
		if (colDataIdType == 1) {
			colVariable = $('#pldVariable').val();
			if (colVariable == null || colVariable == '') {
				Swal.fire({
					text:  'Please enter the variable',
					type:  'warning',
					timer: 1500,
					showConfirmButton: false
				});
			...
			colVariable = $('#selectedFile option:selected').text().trim();
			colDataID = $('#selectedFile option:selected').val();
		}

		if (idJobData == null || idJobData == '') {
			if (colDataIdType == 1 && checkDuplicateData(colVariable)) {
				return;
			}
		} else {
			if (!isNaN(idJobData)) {
				idJob = idJobData;
				indexJob = deleteJobInArrayByID(idJobData);
			} else {
				indexJob = deleteJobInArray(idJobData);
			}
			$('#txtIdJobInput').val('');
			if (checkDuplicateData(colVariable)) {
				colVariable = idJobData;
			}
		}
		var idJobList = colVariable;
		var jobValue = {};
		jobValue['variable'] = colVariable;
		jobValue['idJobList'] = idJobList;
		jobValue['type'] = colDataIdType;
		jobValue['dataID'] = colDataID;
		if (idJob > 0) {
			jobValue['id'] = parseInt(idJob);
		}
		// jobs.push(colInputType, colVariable, idJobList, colDataIdType);
		if (indexJob == -1) {
			dataJob.push(jobValue);
		} else {
			dataJob.splice(indexJob, 0, jobValue)
		}
		loadDataJob();
	}

	function appendNewRow(colDataIdType, colVariable, idJob) {
		var colInputType = colDataIdType == 1 ? 'Text' : 'File';
		$('#tblPlaybookInput2 > tbody:last').append('<tr class="notstandard">'
			+ '<td style="text-align:center; vertical-align: middle" class="gettypevalue" data-id="' + colDataIdType + '">' + colInputType
			+ '<input type="hidden" value="' + idJob + '" class="getidjobinput" />'
			+ '</td>'
			+ '<td class="getvariable resizeCol textcen" style="vertical-align: middle;">' + colVariable + '</td>'
			+ '<td class="textcen">'
	...
	function deleteARow() {
		$('.pldDeleteRow').on('click', function () {
			deleteJobInArray($(this).closest('tr').find('.getvariable').text());
			$(this).closest('tr').addClass('hideclass');
			$(this).closest('tr').find('td').eq(3).text('1');
		});
	}

	function checkDuplicateData(nameVariable) {
		for (var n = 0; n < dataJob.length; n++) {
			if (dataJob[n].variable == nameVariable) {
				Swal.fire({
					text:  'Please re-enter the variable. The variable is duplicated',
					type:  'warning',
					timer: 1500,
					showConfirmButton: false
				});
	...
	function deleteJobInArray(name) {
		for (var i = 0; i < dataJob.length; i++) {
			if (dataJob[i].variable === name) {
				dataJob.splice(i, 1);
				return i;
			}
		}
	...
	function deleteJobInArrayByID(name) {
		for (var i = 0; i < dataJob.length; i++) {
			if (dataJob[i].idJobList === parseInt(name)) {
				dataJob.splice(i, 1);
				return i;
			}
		}
	...
	function editARow() {
		$('.pldEditRow').unbind().on('click', function (e) {
			...
			var inputTypeID = row.find('.gettypevalue').attr('data-id');
			// var inputTypeValue = row.find('.gettypevalue').text().trim();
			var variable = row.find('.getvariable').text().trim();
			var idJobInput = row.find('.getidjobinput').val().trim();
			// var drop3 = row.find('.getfilevalue').text().trim();
			// $('#tblAddInput').removeClass('hideclass');


			setTextEdit(variable, inputTypeID, idJobInput);
			//row.remove();
	...
	function setTextEdit(variable, inputTypeValue, idJobInput) {
		$('#selectedType').val(inputTypeValue).trigger('change');
		$('#pldVariable').val(variable);
		$('#txtIdJobInput').val(idJobInput);
	}

	var buttonExport = this.createDiv('btnExport');
	buttonExport.setAttribute("class", "btn btn-light");
	buttonExport.style.margin = 1 + 'px';

	mxEvent.addListener(buttonExport, 'click', function (evt) {
		ui.showDialog(new ExportDialog(ui).container, 365, 280, true, true);
		// var bounds = graph.getGraphBounds();
		//
		// // New image export
		// var xmlDoc = mxUtils.createXmlDocument();
		...
		//
		// var url = '/nera/api/create-image';
		// $.post(url,{'format':'png','filename':'testing','bg':'#ffffff','w':'640','h':'480','xml':param} ,function( data ) {
		// });

	});
	mxUtils.write(buttonExport, 'Export');

	var buttonViewHistory = this.createDiv('btnHistory');
	buttonViewHistory.setAttribute("class", "btn btn-light");
	buttonViewHistory.style.margin = 1 + 'px';
	mxEvent.addListener(buttonViewHistory, 'click', function (evt) {
		disableLeavePageAlert();
		var workflowId = $('#workflowid', parent.document).val();
		window.top.location.href = '../menu/workflow/workflow-history/' + workflowId;
	});
	mxUtils.write(buttonViewHistory, 'View History');

	var buttonClone = this.createDiv('btnClone');
	buttonClone.setAttribute("class", "btn btn-light");
	buttonClone.style.margin = 1 + 'px';
	mxEvent.addListener(buttonClone, 'click', function (evt) {
		disableLeavePageAlert();
		var workflowId = $('#workflowid', parent.document).val();
		window.top.location.href = '../clone/drawing/' + workflowId;
	});
	mxUtils.write(buttonClone, 'Clone');

	var inName = document.createElement('input');
	var inDes = document.createElement('input');
	var inHiddenVersion = document.createElement('input');
	inName.className = "responsiveWorkflowName";
	inDes.className = "responsiveWorkflowName";
	var spanDes = document.createElement('span');
	var spanName = document.createElement('span');
	var spanVersion = document.createElement('span');
	spanName.innerHTML = "Name";
	spanDes.innerHTML = "Description";
	spanVersion.innerHTML = "Version 1";
	mxUtils.write(inDes, 'Description');
	inName.setAttribute('id', 'WorkflowName');
	inDes.setAttribute('id', 'DescriptionName');
	spanVersion.setAttribute('id', 'versionWorkflow');
	inHiddenVersion.setAttribute('id', 'txtVersion');
	inHiddenVersion.type = 'hidden';
	inHiddenVersion.value = '0';

	this.contentRowButtonToolbar = this.createDiv('row');
	this.contentRowButtonToolbar.style.paddingLeft = '12px';

	this.contentColNameToolbar = this.createDiv('col-md-4');
	this.contentColNameToolbar.appendChild(spanName);
	this.contentColNameToolbar.appendChild(inName);

	this.contentColDescriptionToolbar = this.createDiv('col-md-6');
	this.contentColDescriptionToolbar.appendChild(spanDes);
	this.contentColDescriptionToolbar.appendChild(inDes);

	this.contentColVersionToolbar = this.createDiv('col-md-2');
	this.contentColVersionToolbar.appendChild(spanVersion);
	this.contentColVersionToolbar.appendChild(inHiddenVersion);

	this.contentRowButtonToolbar.appendChild(this.contentColNameToolbar);
	this.contentRowButtonToolbar.appendChild(this.contentColDescriptionToolbar);
	this.contentRowButtonToolbar.appendChild(this.contentColVersionToolbar);

	this.contentTextfieldNeraToolbar.appendChild(this.contentRowButtonToolbar);

	// this.contentTextfieldNeraToolbar.appendChild(spanName);
	// this.contentTextfieldNeraToolbar.appendChild(inName);
	// this.contentTextfieldNeraToolbar.appendChild(spanDes);
	// this.contentTextfieldNeraToolbar.appendChild(inDes);

	if ($('#workflowid', parent.document).val() !== '') {
		this.contentButtonNeraToolbar.appendChild(buttonViewHistory);
		this.contentButtonNeraToolbar.appendChild(buttonClone);
	}
	this.contentButtonNeraToolbar.appendChild(buttonJob);
	this.contentButtonNeraToolbar.appendChild(buttonCancel);
	this.contentButtonNeraToolbar.appendChild(buttonDel);
	this.contentButtonNeraToolbar.appendChild(buttonSave);
	this.contentButtonNeraToolbar.appendChild(buttonExport);

	this.contentRowNeraToolbar.appendChild(this.contentTextfieldNeraToolbar);
	this.contentRowNeraToolbar.appendChild(this.contentButtonNeraToolbar);
	this.neraToolbar.appendChild(this.contentRowNeraToolbar);
	this.container.appendChild(this.neraToolbar);
	// Creates the sidebar
...
EditorUi.prototype.destroy = function () {
	...
	var c = [this.menubarContainer, this.toolbarContainer,
