...
var Graph = function(container, model, renderHint, stylesheet, themes)
{
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		function createTextAreaField(graph, form, cell, attribute)
		{
			var input = form.addTextarea(attribute.nodeName + ':', attribute.nodeValue,10);
			var applyHandler = function()
			{
				var newValue = input.value || '';
				var oldValue = cell.getAttribute(attribute.nodeName, '');
		 ...
		 * Creates the textfield for the given property.
		 */
		function createTextFields(graph, form, cell, attribute,tr)
		{

			var input = document.createElement('input');
			tr = form.addTexts(tr,input,attribute.nodeName+':', attribute.nodeValue,'text');
			var applyHandler = function()
			{
				var newValue = input.value || '';
				var oldValue = cell.getAttribute(attribute.nodeName, '');
	...
	this.graph.addMouseListener(
		{
			mouseDown: mxUtils.bind(this, function(sender, me)
			{
				...
				else if (!this.isActive())
				{
					...
					{
						$('#properties').empty();
						$('#properties').css("z-index", "-1");
						$('#main').css("z-index", "-1");
						var cell = this.graph.getSelectionCell();


						if(cell != null && cell.style != null && cell.style.split('endArrow').length === 1 && cell.getAttribute('label') != 'Start')
						{
							try
							{
								this.graph.getModel().beginUpdate();
								var style = cell.style;
								cell.setStyle(style.split('fillColor')[0]);
							}
							finally
							{
			...
			{
				var evt = me.getEvent();
				mxUtils.convertPoint(this.graph.container,
					...
					mxUtils.contains(this.bbox, me.getGraphX(), me.getGraphY())))
				{
					...
					if (cell !== null) {
						if (cell.value !== null && cell.value !== '') {
							attrs = cell.value.attributes;
						}
					}
					// var form = new mxForm('Status properties');
					var cells = graph.getModel().root.getChildAt(0).children;
					$.each( cells, function( i, cellex ) {
						idCell = cellex.id;
						if (idSelect !== null && idCell === idSelect && cellex.getAttribute('label') !== 'Start') {
							try {
								graph.getModel().beginUpdate();
								var style = cellex.style;
								cellex.setStyle(style.split('fillColor')[0]);
							}
							finally {
								graph.getModel().endUpdate();
								graph.refresh();
					...
					if(this.graph.view.getState(cell).style.shape === 'label')
					{
						$('#btnSearchPlaybook').unbind().on('click', function(e){
							loadDataAllPlaybook($('#playbookSearch').val().trim(),'');
						});
						try
						{
							graph.getModel().beginUpdate();
						...
						var pbid = attrs[2].nodeValue;
						var html = '<div class="row"><div class="col-4"><button class="openbtn" id="btnClosetoggle" onclick="closeNav()"><i class="la la-angle-right"></i></button></div>';
						html += '<div class="col-8"><label class="font-workflow-header">Playbook Properties<label></div><div class="col-12">';
						html += "<table><tr><td  width='40%' style='text-align: center;'>Playbook name  :</td><td width='60%' style='text-align: center;'>"+attrs[0].nodeValue+"<a class='btn btn-hover-brand btn-icon btn-pill btn-link' href='#'><li class='la la-edit'></li></a></td></tr><tr><td width='40%' style='text-align: center;'>Alias</td><td width='60%' style='text-align: center;'><input name='playbookname' value='' maxlength='10'/></td></tr></table>";
						// html += "<table><tr><td>PLaybook name  :</td><td>"+attrs[0].nodeValue+"</td></tr><tr><td>Alias</td><td><input name='playbookname' value=''/></td></tr></table>"
						html +="</div></div>";

						var childNodes;
						if(cell.value.getElementsByTagName('PlaybookInput')[0] != null) {
							childNodes = cell.value.getElementsByTagName('PlaybookInput')[0];
						} else {
							childNodes = cell.value.getElementsByTagName('playbookinput')[0];
						}
						if(childNodes != null && childNodes.childNodes.length > 0)
						{
							html +="<div class='col-12' style='margin-top: 10px;'><h5>Playbook Input</h5></div>";
							html +="<div class='form-group row'><div class='col-12'><table id='playbooktbt' class='table' style='border:1px solid #ccc!important'>";
							html +="<thead><tr style='background-color: #FFFFFF;'><th width='30%' style='text-align: center;'>Variable</th><th width='30%' style='text-align: center;'>Manual input</th><th width='40%' style='text-align: center;'>Assigned Value</th></tr></thead>";
							html +="<tbody>";
							for(var j =0;j<childNodes.childNodes.length;j++)
							{

								var jobs = [];
								$('table#tblPlaybookInput2 tbody tr.notstandard').each(function(e)
								{
									var job = {};
									job['key'] = $(this).find('td').eq(1).text();
									job['value'] = 'JobInput.'+$(this).find('td').eq(1).text();
									jobs.push(job);
								});
								var source = [];
								if(cell.getEdgeCount()>=1)
								{
									for(var k =0;k<cell.getEdgeCount();k++)
									{
										var OutputPlaybook = cell.getEdgeAt(k).source.value.getElementsByTagName('PlaybookOutput')[0]
										if(OutputPlaybook != null && cell.getAttribute('pbid') !== cell.getEdgeAt(k).source.getAttribute('pbid') )
										{
											for(var l =0;l<OutputPlaybook.childNodes.length;l++)
											{
												var pb = {};
												var thirdChildNodesex = OutputPlaybook.childNodes[l].attributes;
												pb['key'] = thirdChildNodesex[1].nodeValue;
												pb['value'] = cell.getEdgeAt(k).source.getAttribute('label')+'.'+thirdChildNodesex[1].nodeValue;
												source.push(pb);
											}
										}
									}
								...
								var thirdChildNodes = childNodes.childNodes[j].attributes;

								if(thirdChildNodes[1].nodeValue === '') {
									source.push({'key':'Default value','value':thirdChildNodes[1].nodeValue});
								} else {
									source.push({'key':'Default value','value':'Default value'});
								}

								var all = $.merge(source,jobs);
								if(thirdChildNodes[3].nodeValue === 'true')
								{
									html+="<tr style='background:#bf4040'>";
									html+="<td width='30%' style='text-align: center;'>"+thirdChildNodes[0].nodeValue+"</td>";

									if(thirdChildNodes[2].nodeValue === 'true')
									{
										html+="<td width='30%' style='text-align: center;'><input class='chkPlaybook' checked type ='checkbox' id='manualinput_"+j+"' value='"+thirdChildNodes[2].nodeValue+"'/></td>";
										html+="<td width='40%' style='text-align: center;'>";
										html+="<select id='value_"+j+"' style='display:none' class='playbookinput form-control form-control-sm'>";
										$.each(all,function(i,value){
											html+= "<option value='"+value['key']+"' >"+value['value']+"</option>";
										});
										html+= "</select>";
										html+= "<input id='value_"+j+"'  class='playbooktext form-control' value='"+thirdChildNodes[1].nodeValue+"'/>";
										html+="</td>";
									}
									else
									{
										html+="<td width='30%' style='text-align: center;'><input class='chkPlaybook' id='manualinput_"+j+"'  type ='checkbox' value='"+thirdChildNodes[2].nodeValue+"'/></td>";
										html+="<td width='40%' style='text-align: center;'>";
										html+="<select id='value_"+j+"' class='playbookinput form-control form-control-sm'>";
										$.each(all,function(i,value){
											if(value['key'] === thirdChildNodes[4].nodeValue)
											{
												html+= "<option selected value='"+value['key']+"' >"+value['value']+"</option>";
											}
											else
											{
												html+= "<option value='"+value['key']+"' >"+value['value']+"</option>";
										...
										html+= "</select>";
										html+= "<input id='value_"+j+"' style='display:none' class='playbooktext form-control' value=''/>";
										html+="</td>";
									}
								...
								else
								{
									html+="<tr>";
									html+="<td width='30%' style='text-align: center;'>"+thirdChildNodes[0].nodeValue+"</td>";
//								html+="<td width='30%' style='text-align: center;'><input class='chkPlaybook' type ='checkbox' value='"+thirdChildNodes[2].nodeValue+"'/></td>";
//								html+="<td width='40%' style='text-align: center;'><select id='value_"+j+"' class='playbookinput form-control form-control-sm'></select><input id='value_"+j+"' style='display:none' class='playbooktext' value=''/>"
									if(thirdChildNodes[2].nodeValue === 'true')
									{
										html+="<td width='30%' style='text-align: center;'><input class='chkPlaybook' checked id='manualinput_"+j+"' type ='checkbox' value='"+thirdChildNodes[2].nodeValue+"'/></td>";
							...
							html +="<div class='col-12' style='margin-top: 10px;'><label class='font-workflow-content'>Playbook Input Empty</label></div>";
							html +="<div class='form-group row'><div class='col-12'></div></div>";
						}

						var childNodesOut;
						if(cell.value.getElementsByTagName('PlaybookOutput')[0] != null) {
							childNodesOut = cell.value.getElementsByTagName('PlaybookOutput')[0];
						} else {
							childNodesOut = cell.value.getElementsByTagName('playbookoutput')[0];
						}
						if(childNodesOut != null && childNodesOut.childNodes.length > 0)
						{
							html +="<div class='col-12' style='margin-top: 10px;'><h5>Playbook Output</h5></div>";
							html +="<table class='table'>";
							html +="<tr style='background-color: #FFFFFF;'><th width='40%' style='text-align: center;'>Variable</th><th width='60%' style='text-align: center;'>Value</th> </tr>";
							for(var j =0;j<childNodesOut.childNodes.length;j++)
							{
								var thirdChildNodes = childNodesOut.childNodes[j].attributes;
								html+="<tr>";
								html+="<td width='40%' style='text-align: center;'>"+childNodesOut.childNodes[j].getAttribute('variable')+"</td>";
								html+="<td width='60%' style='text-align: center;'>"+childNodesOut.childNodes[j].getAttribute('default_value')+"</td>";
								html+="</tr>";
							}
							html +="</table>";

						} else {
							html +="<div class='col-12'><label class='font-workflow-content'>Playbook Output Empty</label></div>";
							html +="<div class='form-group row'><div class='col-12'></div></div>";
						}

						// end playbook input
						html +="<table>";
						html +="<tr><td width='100%' style='text-align: center;'><a id='detailPLaybook' href='#'>View more detail</a></td><tr>";
						html +="</table>";
						...
						$("#btnSelect").unbind().on('click',function(e){
							var id = $('tbody tr.active').find('td').eq(0).find('#dataid').text();
							var json = {id: id};
							$.ajax({
								type: 'GET',
								...
								success: function (data) {
									var playbook = document.createElement('Playbook');
									playbook.setAttribute('label', data.name);
									playbook.setAttribute('alias', "");
									playbook.setAttribute('pbid', data.id);
									var playbookinput = document.createElement('PlaybookInput');
									playbookinput.setAttribute('name','Playbook Input');
									for(var i =0;i<data.playbookInput.length;i++)
									{
										var inputelement = document.createElement('PLaybookInputElement');
										// inputelement.setAttribute('index',i);
										inputelement.setAttribute('variable',data.playbookInput[i].variable);
										inputelement.setAttribute('value_'+i,data.playbookInput[i].value);
										inputelement.setAttribute('manualinput_'+i,false);
										inputelement.setAttribute('manda',data.playbookInput[i].mandatory);
										inputelement.setAttribute('lalelvl_'+i,'Default value');
										playbookinput.appendChild(inputelement);
									}
									var playbookoutput = document.createElement('PlaybookOutput');
									playbookoutput.setAttribute('name','Playbook Output');
									for(var i=0;i<data.playbookOutput.length;i++)
									{
										var outputelement = document.createElement('PlaybookOutputElement');
										outputelement.setAttribute('index',i);
										outputelement.setAttribute('variable',data.playbookOutput[i].variable);
										outputelement.setAttribute('default_value',data.playbookOutput[i].value);
										playbookoutput.appendChild(outputelement);
									}

									playbook.appendChild(playbookinput);
									playbook.appendChild(playbookoutput);

									graph.model.setValue(graph.getSelectionCell(),playbook);
									...
									var html = '<div class="row"><div class="col-4"><button class="openbtn" id="btnClosetoggle" onclick="closeNav()"><i class="la la-angle-right"></i></button></div>';
									html += '<div class="col-8"><label class="font-workflow-header">Playbook Properties<label></div><div class="col-12">';
									html += "<table><tr><td  width='40%' style='text-align: center;' class='font-workflow-content'>Playbook name  :</td><td width='60%' style='text-align: center;'>"+data.name+"<a class='btn btn-hover-brand btn-icon btn-pill btn-link' href='#'><li class='la la-edit'></li></a></td></tr><tr><td width='40%' style='text-align: center;' class='font-workflow-content'>Alias</td><td width='60%' style='text-align: center;'><input  name='playbookname' value='' maxlength='10'/></td></tr></table>"
									html +="</div></div>";

									if(data.playbookInput.length > 0) {
										html +="<div class='col-12' style='margin-top: 10px;'><label class='font-workflow-content'>Playbook Input</label></div>";
										html +="<div class='form-group row'><div class='col-12'><table id='playbooktbt' class='table' style='border:1px solid #ccc!important'>";
										html +="<thead><tr style='background-color: #FFFFFF;'><th width='30%' style='text-align: center;'>Variable</th><th width='30%' style='text-align: center;'>Manual input</th><th width='40%' style='text-align: center;'>Assigned Value</th></tr></thead>";
										var childNodes = cell.value.getElementsByTagName('PlaybookInput')[0];
										if(childNodes != null)
										{
											for(var j =0;j<childNodes.childNodes.length;j++)
											{
												// var thirdChildNodes = childNodes.childNodes[j].attributes;
												if(childNodes.childNodes[j].getAttribute('manda') === 'true')
												{
													html+="<tr class='content' style='background:#bf4040'>";
													html+="<td width='30%' style='text-align: center;'><label>"+childNodes.childNodes[j].getAttribute('variable')+"</label></td>";
													html+="<td width='30%' style='text-align: center;'><input class='chkPlaybook'  id='manualinput_"+j+"' type ='checkbox' value='"+childNodes.childNodes[j].getAttribute('manualinput_'+j)+"'/></td>";
													html+="<td width='40%' style='text-align: center;'><select id='value_"+j+"'  class='playbookinput form-control form-control-sm'><option selected='true' value='"+childNodes.childNodes[j].getAttribute('manualinput_'+j)+"'>"+childNodes.childNodes[j].getAttribute('lalelvl_'+j)+"</option></select><input id='value_"+j+"' style='display:none' class='playbooktext form-control' value=''/>"
													html+="</tr>";
												}
												else
												{
													...
													html+="<td width='30%' style='text-align: center;'><label>"+childNodes.childNodes[j].getAttribute('variable')+"</label></td>";
													html+="<td width='30%' style='text-align: center;'><input class='chkPlaybook'  id='manualinput_"+j+"' type ='checkbox' value='"+childNodes.childNodes[j].getAttribute('manualinput_'+j)+"'/></td>";
													html+="<td width='40%' style='text-align: center;'><select id='value_"+j+"' class='playbookinput form-control form-control-sm'><option selected='true' value='"+childNodes.childNodes[j].getAttribute('manualinput_'+j)+"'>"+childNodes.childNodes[j].getAttribute('lalelvl_'+j)+"</select><input id='value_"+j+"' style='display:none' class='playbooktext form-control' value=''/>"
													html+="</tr>";
												}
									...
									if(data.playbookOutput.length > 0) {
										var childNodesOut = cell.value.getElementsByTagName('PlaybookOutput')[0];
										if(childNodesOut != null)
										{
											html +="<div class='col-12 font-workflow-content' style='margin-top: 10px;'><label>Playbook Output</label></div>";
											html +="<table class='table font-workflow-content'>";
											html +="<tr style='background-color: #FFFFFF;'><th width='60%' style='text-align: center;'><label>Variable</label></th><th width='40%' style='text-align: center;'><label>Value</label></th></tr>";
											for(var j =0;j<childNodesOut.childNodes.length;j++)
											{
												html+="<tr>";
												html+="<td width='60%' style='text-align: center;'><label>"+childNodesOut.childNodes[j].getAttribute('variable')+"</label></td>";
												html+="<td width='40%' style='text-align: center;'><label>"+childNodesOut.childNodes[j].getAttribute('default_value')+"</label></td>";
												html+="</tr>";
											}
											html +="</table>";
									...
									// end playbook input
									html +="<table>";
									html +="<tr><td width='100%' style='text-align: right;'><a id='detailPLaybook' href='#'><label>View more detail</label></a></td><tr>";
									html +="</table>";
									$('#playbookworkflow').modal('hide');
									$('#playbookworkflowdetail').modal('hide');
									div.append(html);
									$( "#properties" ).mouseleave(function( event ) {
										var count = 0;
										$('table#playbooktbt tbody tr').each(function(e)
										{

											if($(this).find('td').eq(1).find('.chkPlaybook').is(":checked"))
											{

												var seval = $(this).find('td').eq(2).find('input').val();
												if(seval === "")
												{
													count +=1;
													...
													Swal.fire({
														title: 'Required value for '+$(this).find('td').eq(0).text(),
														text:  "Please make assignment to mandatory input",
														type:  'error',
														timer: 1000,
														showConfirmButton: false
													});
													$('#geEditor').append('<div class="modal-backdrop show"></div>');
												}
												else
												{
													if(count === 0)
											...
											{
												var seval = $(this).find('td').eq(2).find('select').val();
												if(seval === "")
												{
													count +=1;
													$('.modal-backdrop').remove();
									...
									div.css("z-index", "1");
									if(div.width() == 0) {
										div.addClass('responsive-width-properties');
										div.removeClass('responsive-width-button');
									}

									$('#detailPLaybook').unbind().on('click',function(e){
										var playbookid = data.id;
										showUpdatePopup(playbookid);
									});
									$('.chkPlaybook').change(function() {
										var newValue = false;
										if($(this).is(":checked"))
										{
											newValue = true;
											$(this).parent().parent().find('td').eq(2).find('select').hide();
											$(this).parent().parent().find('td').eq(2).find('input').show();
										}
										else
										{
											$(this).parent().parent().find('td').eq(2).find('select').show();
											$(this).parent().parent().find('td').eq(2).find('input').hide();
										}
										var attr = $(this).attr('id');
										graph.getModel().beginUpdate();
										try
										{
											var edit = new mxCellAttributeChangeEX(
												cell, attr,
												newValue,'PLaybookInputElement');
											graph.getModel().execute(edit);
											// graph.updateCellSize(cell);
										}
										finally
									...
									var select = $('.playbookinput');
//								select.find('option').remove();
									var jobs = [];

									$('table#tblPlaybookInput2 tbody tr.notstandard').each(function(e)
									{
										jobs.push($(this).find('td').eq(1).text());
									});
									var source = [];
									...
									if(cell.getEdgeCount()>1)
									{
										...
										{
											var OutputPlaybook = cell.edges[i].source.value.getElementsByTagName('PlaybookOutput');
											var playbookN = cell.getAttribute('label');
											if(OutputPlaybook != null && childNodesOut.length>0)
											{
												for(var j =0;j<childNodesOut.childNodes.length;j++)
												{
													var obj = {}
													var thirdChildNodes = childNodesOut.childNodes[j].attributes;
													obj['key'] = playbookN;
													obj['value'] = thirdChildNodes[1].nodeValue;
													source.push(obj);
												}
											}
										}
...
//									var all = $.merge(jobs,source);
									$.each(jobs,function(i,value){
										select.append('<option value="'+value+'" >Jobinput.'+value+'</option>');
									});
									$.each(source,function(i,value){
										select.append('<option value="'+value['value']+'" >'+value['key']+value['value']+'</option>');
									});
									$('.playbookinput').unbind().on('click',function(){
										$(this).find(":selected").text();
										var attr1 = $(this).attr('id');
										var attr2 = 'lalelvl_'+$(this).attr('id').split('_')[1];
										...
										try
										{
											var edit1 = new mxCellAttributeChangeEX(
												cell, attr1,
												$(this).find(":selected").text(),'PLaybookInputElement');
											graph.getModel().execute(edit1);
											var edit2 = new mxCellAttributeChangeEX(
												cell, attr2,
												$(this).find(":selected").val(),'PLaybookInputElement');
									...
									$("input[name='playbookname']").change(function() {
										var newValue = this.value ;
										var attr = 'label';
										if(newValue !== '')
										{
						...
						$( "#properties" ).mouseleave(function( event ) {
							...
							$('table#playbooktbt tbody tr').each(function(e)
							{
								...
								{
									var seval = $(this).find(":selected").text();
									if(seval === "" && $(this).css('background-color') == 'rgb(191, 64, 64)')
									{
										count +=1;
										$('.modal-backdrop').remove();
										div.css("z-index", "9999");
					...
					else if (this.graph.view.getState(cell).style.shape === 'triangle')
					{
						// change color
						try
						{
						...
						if(attrs != null)
						{
							...
							var approved = '';
							if(attrs != '') {
								nodeValue = attrs[0].nodeValue;
								approved = attrs[1].nodeValue;
							}

							var html = '<div class="row"><div class="col-4"><button class="openbtn" id="btnClosetoggle" onclick="closeNav()"><i class="la la-angle-right"></i></button></div>';
							html += '<div class="col-8"><label class="font-workflow-header">Approval Properties<label></div><div class="col-12">';
							html += "<table><tr><td  width='40%' style='text-align: center;'>" +
								"<label> Name  :<span class='redmarker'>*</span></label></td><td width='60%' style='text-align: center;'><input style='width:260px' class='form-control' autofocus  name='ipapprovedname' maxlength='10' value='"+nodeValue+"'  /></td></tr>" +
								"<tr><td  width='40%' style='text-align: center;'><label> Approver  :</label></td><td width='60%' style='text-align: center;'><textarea class='form-control' readonly='' rows='3' id='approver' style='min-height: 300px; background: #f8f8f8'>" + approved + "</textarea></td></tr>";
							html += "<tr><td width='40%'></td><td width='60%'><button type='button' class='btn btn-add-default' id='btnApprovalProperties'>Select Approval</button></td></tr></table>";
							html +="</div></div>";
							div.append(html);
							div.css("z-index", "1");
							if(div.width() == 0) {
						...
						$('#btnApprovalProperties').unbind().on('click',function(e){
							$('#approvedSearch').val('');
							var arrApprover = [];
							if(typeof $("#approver").val() !== 'undefined' || $("#approver").val() != '') {
								arrApprover = $("#approver").val().split(',');
							}
							$('#playbookApproveTable').empty();
							var htmlct = '';
							htmlct += '<thead><tr class="textcen"><th width="10%"><input type="checkbox" value="" name="checkAllData" onClick="selectAll(this)"/></th><th width="40%">Approver</th></tr></thead>';
							htmlct += '<tbody>';
							var totalChecked = 0;
							var totalData = 0;
							$.ajax({
								type: 'POST',
								url: '/nera/api/get-all-user-approve',
								data: {approved: ''},
								success: function (data) {
									if(data != null) {
									...
									for(var n=0; n<data.length; n++) {
										htmlct += "<tr class='row-table-data'>";
										htmlct += '<td style="text-align: center;"><input class="groupApproval" type="checkbox" value=""';
										if(arrApprover.length > 0) {
											for (var a = 0; a<arrApprover.length; a++) {
												if(data[n] == arrApprover[a].trim()) {
													htmlct += ' checked';
													totalChecked++;
												}
											}
										}
										htmlct +=  '/><span style="display:none">'+data[n]+'</span></td>';
										htmlct += '<td><label>'+data[n]+'</label></td>';
										htmlct += "</tr>";
									}
									htmlct += '</tbody>';
									$('#playbookApproveTable').append(htmlct);
									if (totalChecked == totalData) {
										$("input[name='checkAllData']:checkbox").prop('checked',true);
									}
									$('#playbookworkflowApproveStatus').modal('show');
								}
							});

						});
						$("#btnSelectApprove").unbind().on('click',function(e){
//					approval.setAttribute('label', 'Approval');
							var approvedName = '';
							if(typeof $("input[name='ipapprovedname']").val() !== 'undefined') {
								approvedName = $("input[name='ipapprovedname']").val();
							}
							$('#properties').empty();
							var approver = '';
							$('table#playbookApproveTable tbody tr').each(function(e){
								if($(this).find('td').eq(0).find('input').is(':checked') && $(this).find('td').eq(0).find('span').text() != ''){
									var approverdata = $(this).find('td').eq(0).find('span').text();
							...
							// approver = approver.substring(1,approver.length);

							// graph.model.setValue(graph.getSelectionCell(),approval);
							var div = $('#properties');
							var html = '<div class="row"><div class="col-4"><button class="openbtn" id="btnClosetoggle" onclick="closeNav()"><i class="la la-angle-right"></i></button></div>';
							html += '<div class="col-8"><label class="font-workflow-header">Approval Properties<label></div><div class="col-12">';
							html += "<table><tr><td  width='40%' style='text-align: center;'>" +
								"<label class='font-workflow-content'> Name  :<span class='redmarker'>*</span></label></td><td width='60%' style='text-align: center;'><input style='width:260px' class='form-control'  name='ipapprovedname' maxlength='10' value='"+approvedName+"' autofocus/></td></tr>" +
								"<tr><td  width='40%' style='text-align: center;'><label class='font-workflow-content'> Approver  :</label></td><td width='60%' style='text-align: center;'><textarea class='form-control' readonly='' rows='2' id='approver' style='min-height: 200px; background: #f8f8f8'>" + approver + "</textarea></td></tr>";
							html += "<tr><td width='40%'></td><td width='60%'><button type='button' class='btn btn-add-default' id='btnApprovalProperties'>Select Approval</button></td></tr></table>";
							html +="</div></div>";
							...
							$("input[name='ipapprovedname']").change(function() {
								var newValue = this.value || '';
								var attr = 'label';
								graph.getModel().beginUpdate();
								try
							...
							$('#playbookworkflowApproveStatus').modal('hide');
						});
						$("#btnCancelApprove").unbind().on('click',function(e){
							$('#playbookworkflowApproveStatus').modal('hide');
						});
//					setTimeout(function() { $("input[name='ipapprovedname']").focus( )}, 1000);
//					$("input[name='ipapprovedname']").blur(function() {
//
//						if(this.value === '')
//							{
//
...
//									text:  "Please enter value for the approval name",
//									type:  'error',
//									timer: 2000,
//									showConfirmButton: false
//								});
//								$(this).focus();
//								$('#isError', parent.document).val(1);
//								$('#messageError', parent.document).val('Please enter value for the approval name');
//
//							}
//						else if(this.value !== '')
//							{
//								$('#isError', parent.document).val(0);
//								$('#messageError', parent.document).val('');
//							}
//					});
						$("input[name='ipapprovedname']").change(function() {
							var newValue = this.value || '';
					...
					else if(graph.view.getState(cell).style.shape === 'rhombus')
					{
						try
						{
							graph.getModel().beginUpdate();
						...
						if(attrs != null)
						{
							var div = $('#properties');
							var html = '<div class="row"><div class="col-4"><button class="openbtn" id="btnClosetoggle" onclick="closeNav()"><i class="la la-angle-right"></i></button></div>';
							html += '<div class="col-8"><label class="font-workflow-header">Switch Case Properties<label></div><div class="col-12">';
							html += '<table style="margin-bottom: 15px;">';
							html += '<tr>';
							html += '<td  width="40%" style="text-align: center;"> Name  :<span class="redmarker">*</span></td>';
							html += '<td width="60%" style="text-align: center;"><input style="width:260px" maxlength="10" class="form-control"  name="conditionname" value="'+attrs[0].nodeValue+'"/></td>';
							html += '</tr>';
							html += '</table>';
							html += '</div></div>';
							div.append(html);
							...
							if(cell.getEdgeCount()>=1) {
								...
								$.each(cell.parent.children, function (index, value) {
									if(value.target !== null) {
										cellParent.push(value);
										if(value.source.id === cell.id) {
											cellSource.push(value);
										}
									}
								});
								selectCell(idCell,InputPlaybook,playbookName, cellParent);
								function selectCell(idCell,InputPlaybook,playbookName, cellParent) {
									var idCellNext = '';
									for(var n=0; n<cellParent.length; n++) {
										if(cellParent[n].target.id === idCell) {
											var cellSelect = cellParent[n].source;
											idCellNext = cellSelect.id;
											if(cellSelect.value.nodeName === 'Playbook' || cellSelect.value.nodeName === 'PLAYBOOK') {
												if(cellSelect.value.getElementsByTagName('PlaybookOutput')[0] !== null) {
													InputPlaybook.push(cellSelect.value.getElementsByTagName('PlaybookOutput')[0]);
													playbookName.push(cellSelect.value.attributes[0].nodeValue);
												}
											} else if (cellSelect.value.nodeName === 'playbook' && cellSelect.value.getElementsByTagName('playbookoutput')[0] !== null) {
												InputPlaybook.push(cellSelect.value.getElementsByTagName('playbookoutput')[0]);
												playbookName.push(cellSelect.value.attributes[0].nodeValue);
											}
											cellParent.splice(n,1);
											break;
										}
									}
									if (idCellNext !== '' && idCellNext !== 'star'){
										selectCell(idCellNext,InputPlaybook,playbookName, cellParent);
									}
								}

								for(var i=0;i<cellSource.length;i++)
								{
									var playbooktargetName  =  cellSource[i].target.value.attributes[0].nodeValue;
									var sources = [];
									var jobs = [];
									$('table#tblPlaybookInput2 tbody tr.notstandard').each(function(e)
									{
										var job = {};
										job['key'] = 'Jobinput'+'.'+$(this).find('td').eq(1).text();
										job['value'] = 'Jobinput'+'.'+$(this).find('td').eq(1).text();
										jobs.push(job);

									});

									var htmlex = '<div style="margin-bottom: 20px; border: 1px solid #a29494">';
									htmlex += '<div class="row" style="position: relative; text-align: left;"><div class="col-1"></div><div class="layout-switch-case-border col-6">'+ playbooktargetName +'</div>';
									htmlex += '<div style="margin-top: 10px; margin-left: auto;"><button type="button" class="btn btn-add-default btn-hover-brand" style="margin-top: 5px;" id="btnAddPlaybook_'+i+'"><i class="la la-plus-circle"></i>Add Condition</button></div></div>';
									htmlex += '<div class="form-group row" style="margin-bottom: 0px;">';
									htmlex += '<div class="col-12" style="padding-right: 0px;">';
									htmlex += '<table class="table" id="tblPlaybook'+i+'" name="'+playbooktargetName+'">';
									htmlex += '<thead>';
									htmlex += '<tr>';
									htmlex +=  '<th width="40%" style="text-align: center;">Parameter</th>';
									htmlex += '<th width="15%" style="text-align: center;">Condition</th>';
									htmlex += '<th width="35%" style="text-align: center;">Value</th>';
									htmlex += '<th width="10%" style="text-align: center;"></th>';
									htmlex += '</tr></thead>';
									htmlex += '</table>';
									htmlex += '</div>';
									htmlex += '</div>';
									...
									{
										for(var n=0;n<InputPlaybook.length;n++)
										{
											for(var j =0;j<InputPlaybook[n].childNodes.length;j++)
											{
												var dict = {};
												var thirdChildNodes = InputPlaybook[n].childNodes[j].attributes;
												dict['key'] = playbookName[n]+'.'+thirdChildNodes[1].nodeValue;
												dict['value'] = playbookName[n]+'.'+thirdChildNodes[1].nodeValue;
												sources.push(dict);
											}
										}
									}
									// get cell attributes in here
									var all = $.merge(jobs,sources);
									var index = 1;
									if(cell.value.attributes.length>1 && typeof cell.getAttribute('conditionPlaybook'+i+'_'+index) !== 'undefined')
									{
										while(typeof cell.getAttribute('conditionPlaybook'+i+'_'+index) !== 'undefined') {
											if(cell.getAttribute('conditionPlaybook'+i+'_'+index) === playbooktargetName)
											{
												addRow(all,'tblPlaybook'+i,cell.getAttribute('paramkey'+i+'_'+index,''),cell.getAttribute('conditionkey'+i+'_'+index,''),cell.getAttribute('value'+i+'_'+index,''));
												// var rows = cell.value.attributes[cell.value.attributes.length-1].nodeName.split('_')[1];
												// for(var run =1;run <=rows;run++)
												// {
												// 	addRow(all,'tblPlaybook'+i,cell.getAttribute('paramkey'+i+'_'+run,''),cell.getAttribute('conditionkey'+i+'_'+run,''),cell.getAttribute('value'+i+'_'+run,''));
												// }
											}
											index++;
										}
									...
									{
										if(sources.length > 0) {
											addRow(all,'tblPlaybook'+i,sources[0]['key'],'none','none');
										} else {
											addRow(all,'tblPlaybook'+i,'none','none','none');
										}

									}

									$('#btnAddPlaybook_'+i).click(function(){
										if(all.length > 0) {
											addRow(all,'tblPlaybook'+this.id.split('_')[1],all[0]['key'],'none','none');
										} else {
											addRow(all,'tblPlaybook'+this.id.split('_')[1],'none','none','none');
										}
										listeningAssign();
									});

									$(document).on('click', '.deleteRowTable' , function(e) {
										removeRow($(this), e,this.name);
									});
									function addRow(arrParameter,tableName,selectedVal,selectedOp,value) {
										var arrHead = ['Parameter', 'Condition', 'Value', ''];
										var empTab = document.getElementById(tableName);
										var rowCnt = empTab.rows.length;        // GET TABLE ROW COUNT.
										var tr = empTab.insertRow(rowCnt);      // TABLE ROW.

										for (var head = 0; head < arrHead.length; head++) {
											var td = document.createElement('td');          // TABLE DEFINITION.
											td = tr.insertCell(head);

											if (head == 0) {
												var elementSelect = document.createElement('select');
												var arraySelect = arrParameter;
												elementSelect.setAttribute('style', 'width:100%');
												elementSelect.setAttribute('name','parameter');
												td.appendChild(elementSelect);
												for (var n = 0; n < arraySelect.length; n++) {
													var option = document.createElement('option');
													option.value = arraySelect[n]['key'];
													option.text = arraySelect[n]['value'];
													elementSelect.appendChild(option);
												}
												if(selectedVal !== 'none')
												{
													elementSelect.value = selectedVal;
												}

											} else if(head == 1) {
												// CREATE AND ADD TEXTBOX IN EACH CELL.
												var elementSelect = document.createElement('select');
												var arraySelect = ["=","Like","Not like","<",">","<=",">="];
												elementSelect.setAttribute('style', 'width:100%');
												elementSelect.setAttribute('name','condition');
												td.appendChild(elementSelect);
												...
												if(selectedOp !== 'none')
												{
													elementSelect.value = selectedOp;
												}
												...
												if(value !== 'none')
												{
													elementText.setAttribute('value', value);
												}
												else
												{
													elementText.setAttribute('value', '');
												}
												elementText.setAttribute('style', 'width:100%');
												elementText.setAttribute('name','assign');
												...
												button.setAttribute('name',tableName);
												button.setAttribute('class', 'btn btn-hover-danger btn-icon btn-pill deleteRowTable');
												// ADD THE BUTTON's 'onclick' EVENT.
												td.appendChild(button);
												// SET tag I ATTRIBUTE.
												var tagI = document.createElement('i');
												tagI.setAttribute('class', 'la la-trash');
												button.appendChild(tagI);
											}
										}
									}
									// DELETE TABLE ROW.
									function removeRow(oButton, e,tableName) {
										var empTab = document.getElementById(tableName);
										empTab.deleteRow(oButton[0].parentNode.parentNode.rowIndex);
							...
							{
								if(cell.value.attributes.length >1) {

								}
							}
							listeningAssign();
							function listeningAssign()
							{
								$('input[name="assign"]').blur(function(){
									if(this.value === '')
									{
										Swal.fire({
											title: 'Required value',
											text:  "Please fill in the Parameter and Value",
											type:  'error',
											timer: 2000,
											showConfirmButton: false
										});
								...
								$('input[name="assign"]').change(function(){
									$('input[name="assign"]').each(function(){
										var row = $(this).parent().parent().parent().children().index($(this).parent().parent());
										var thiz = $(this).parent().parent().find('td');
										var paramvalue = thiz.eq(0).find('select').find(":selected").text();
										var paramkey = thiz.eq(0).find('select').find(":selected").val();
										var conditionvalue = thiz.eq(1).find('select').find(":selected").text();
										var conditionkey = thiz.eq(1).find('select').find(":selected").val();
										var assign = thiz.eq(2).find('input').val();
										var tableindex = $(this).parent().parent().parent().parent().attr('id').split('tblPlaybook')[1];
										var newValue1 = assign;
										var attr1 = 'value'+tableindex+'_'+row;
										var attr2 = 'paramvalue'+tableindex+'_'+row;
										var attr3 = 'paramkey'+tableindex+'_'+row;
										var attr4 = 'conditionvalue'+tableindex+'_'+row;
										var attr5 = 'conditionkey'+tableindex+'_'+row;
										var attr6 = 'conditionPlaybook'+tableindex+'_'+row;
										graph.getModel().beginUpdate();
										try
										{
											var edit = new mxCellAttributeChange(
												...
												newValue1);
											graph.getModel().execute(edit);
											graph.getModel().execute(new mxCellAttributeChange(cell, attr2,paramvalue));
											graph.getModel().execute(new mxCellAttributeChange(cell, attr3,paramkey));
											graph.getModel().execute(new mxCellAttributeChange(cell, attr4,conditionvalue));
											graph.getModel().execute(new mxCellAttributeChange(cell, attr5,conditionkey));
											graph.getModel().execute(new mxCellAttributeChange(cell, attr6,$(this).parent().parent().parent().parent().attr('name')));
										}
										finally
										{
											graph.getModel().endUpdate();
								...
								$('select[name="parameter"]').change(function(){
									$('select[name="parameter"]').each(function(){
										var row = $(this).parent().parent().parent().children().index($(this).parent().parent());
										var thiz = $(this).parent().parent().find('td');
										var paramvalue = thiz.eq(0).find('select').find(":selected").text();
										var paramkey = thiz.eq(0).find('select').find(":selected").val();
							...
							$('input[name="conditionname"]').change(function()
							{
								var newValue = this.value || '';
								var attr = 'label';
								graph.getModel().beginUpdate();
					...
					else if (this.graph.view.getState(cell).style.shape === 'ellipse')
					{

						if(cell.getAttribute('label') !== 'Start')
						{
							try
							{
								graph.getModel().beginUpdate();
						...
						if(attrs != null)
						{
							if(attrs[0].nodeValue !== 'Start' && attrs[0].nodeValue !== 'End')
							{
								var html = '<div class="row"><div class="col-4"><button class="openbtn" id="btnClosetoggle" onclick="closeNav()"><i class="la la-angle-right"></i></button></div>';
								html += '<div class="col-8"><label class="font-workflow-header">Status Properties<label></div><div class="col-12">';
								html += "<table><tr><td  width='40%' style='text-align: center;'>Name  :<span class='redmarker'>*</span></td><td width='60%' style='text-align: center;'><input class='form-control' name='ipstatusname' value='"+attrs[0].nodeValue+"'/></td></tr><tr><td width='40%' style='text-align: center;'>The Status</td><td width='60%' style='text-align: center;'><textarea id='txtstatus' class='form-control' value='"+attrs[1].nodeValue+"' rows='10' cols='22'>"+attrs[1].nodeValue+"</textarea></td></tr></table>"
								html +="</div></div>";
								div.append(html);
								div.css("z-index", "1");
								if(div.width() == 0) {
						...
						$("input[name='ipstatusname']").change(function() {
							var newValue = this.value || '';
							var attr = 'label';
							graph.getModel().beginUpdate();
							try
