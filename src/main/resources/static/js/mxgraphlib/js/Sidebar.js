...
function selectAll(nameUser) {
	if(nameUser.checked) {
		$('.groupApproval').each(function(){
			this.checked = true;
		});
	} else {
		$('.groupApproval').each(function(){
...
Sidebar.prototype.addGeneralPalette = function(expand)
{
	...
	var fns = [
	 	this.createVertexTemplateEntry('rounded=0;whiteSpace=wrap;html=1;', 120, 60, '', '    Playbook', null, null, 'rect rectangle box'),
 		...
 		// this.createVertexTemplateEntry('ellipse;whiteSpace=wrap;html=1;', 120, 80, '', 'Update Status', null, null, 'oval ellipse state'),
		...
		//this.createVertexTemplateEntry('ellipse;whiteSpace=wrap;html=1;aspect=fixed;', 80, 80, '', 'Circle', null, null, 'circle'),
	 	...
	 	this.createVertexTemplateEntry('rhombus;whiteSpace=wrap;html=1;', 80, 80, '', 'Switch case', null, null, 'diamond rhombus if condition decision conditional question test'),
	 	...
	 	this.createVertexTemplateEntry('triangle;whiteSpace=wrap;html=1;', 60, 80, '', 'Approve', null, null, 'triangle logic inverter buffer'),
	    ...
	    //this.createVertexTemplateEntry('shape=callout;whiteSpace=wrap;html=1;perimeter=calloutPerimeter;', 120, 80, '', 'Callout', null, null, 'bubble chat thought speech message'),
	 	...
	 	this.createEdgeTemplateEntry('endArrow=classic;html=1;', 50, 50, '', 'Connector', null, lineTags + 'directional directed')
	];
...
//	elt.style.padding = this.thumbPadding + 'px';
	elt.style.width = 180+ 'px';
	elt.style.height = (this.thumbHeight + border + 3) + 'px';
	elt.style.padding = this.thumbPadding + 'px';
	...
	try {
		...
		if (title.outerHTML) {
			elt.innerHTML = title.outerHTML;
		}
		else if(title) {
			elt.innerHTML = elt.innerHTML + "<div style='position: absolute; left: 70px; top:10px'>" +title + '</div>';
		}
	}
...
Sidebar.prototype.createDragSource = function(elt, dropHandler, preview, cells, bounds)
{
	...
	var dragSource = mxUtils.makeDraggable(elt, this.editorUi.editor.graph, mxUtils.bind(this, function(graph, evt, target, x, y)
	{
		...
		if (this.editorUi.hoverIcons != null)
		{
			...
			$('#properties').empty();
			var errors = {};
			if(graph.view.getState(graph.getSelectionCell()).style.shape === 'label')
			{
				loadDataAllPlaybook('','');
				$('#btnCancel').addClass('hideclass');
					...
					var json = {id: id};
						$.ajax({
							...
							success: function (data) {
								...
								var playbookoutput = doc.createElement('PlaybookOutput');
								playbookoutput.setAttribute('name','Playbook Output');
								for(var i=0;i<data.playbookOutput.length;i++)
								{
									var outputelement = doc.createElement('PlaybookOutputElement');
								...
								if(data.playbookInput.length > 0) {
									...
									if(childNodes != null)
									{
										for(var j =0;j<childNodes.childNodes.length;j++)
										{
											...
											var valueInput = 'Default value';
											if(childNodes.childNodes[j].getAttribute('value_'+j) === '' ) {
												valueInput = childNodes.childNodes[j].getAttribute('value_'+j);
											}
											if(childNodes.childNodes[j].getAttribute('manda') === 'true')
											{
												...
												html+="<td width='40%' style='text-align: center;'><select id='value_"+j+"'  class='playbookinput form-control form-control-sm'><option selected='true' value='"+childNodes.childNodes[j].getAttribute('manualinput_'+j)+"'>"+valueInput+"</option></select><input id='value_"+j+"' style='display:none' class='playbooktext form-control' value=''/>"
												html+="</tr>";
											}
											else
											{
...
//								select.find('option').remove();
									...
									if(cell.getEdgeCount()>1)
									{
										...
										{
											var OutputPlaybook = cell.getCellAt(i).source.value.getElementsByTagName('PlaybookOutput')[0]
											var playbookN = cell.getAttribute('label');
											if(OutputPlaybook != null)
											{
												for(var j =0;j<OutputPlaybook.childNodes.length;j++)
												{
													var obj = {}
...
//			var arrParameter = ['tien','phuong'];
			// condition
			if(graph.view.getState(graph.getSelectionCell()).style.shape == 'rhombus')
			{
				var Condition = doc.createElement('Condition');
				Condition.setAttribute('label', '');
				graph.model.setValue(graph.getSelectionCell(),Condition);
				...
				var html = '<div class="row"><div class="col-4"><button class="openbtn" id="btnClosetoggle" onclick="closeNav()"><i class="la la-angle-right"></i></button></div>';
					...
					html += '<table stlye="margin-bottom: 15px;">';
					html += '<tr>';
					html += '<td  width="40%" class="font-workflow-content" style="text-align: center;"> Name  :<span class="redmarker">*</span></td>';
					html += '<td width="60%" style="text-align: center;"><input style="width:260px" class="form-control " required maxlength="10" name="conditionname" value="" /></td>';
					html += '</tr>';
					html += '</table>';
					html += '</div></div>';
				div.append(html);
...
//					graph.refresh();
//				}
//				$('input[name="conditionname"]').attr('autofocus',true);
//				setTimeout(function() { $('input[name="conditionname"]').focus( )}, 1000);
//				
//				$('input[name="conditionname"]').blur(function() {
//					
//					if(this.value === '')
//						{
//							
...
//								text:  "Please enter value for the switch case name",
//								type:  'error',
//								timer: 2000,	
//								showConfirmButton: false
//							});
//							$(this).unbind().on('focus',function(){});
//							$('#isError', parent.document).val(1);
//							$('#messageError', parent.document).val('Please enter value for the switch case name');
//							
//						}
//				});
			...
			// connector
			if(graph.view.getState(graph.getSelectionCell()).style.shape == 'connector')
			{
				var UserObject = doc.createElement('UserObject');
				UserObject.setAttribute('name', 'connector');
				graph.model.setValue(graph.getSelectionCell(),UserObject);
			}
			//  stattus
			if(graph.view.getState(graph.getSelectionCell()).style.shape == 'ellipse')
			{
				// create user object
				var status = doc.createElement('Status');
				status.setAttribute('label',"");
				status.setAttribute('thestatus',"");
				graph.model.setValue(graph.getSelectionCell(),status);
				var div = $('#properties');
				var cell = graph.getSelectionCell();
				var html = '<div class="row"><div class="col-4"><button class="openbtn" id="btnClosetoggle" onclick="closeNav()"><i class="la la-angle-right"></i></button></div>';
				html += '<div class="col-8"><label class="font-workflow-header">Status Properties<label></div><div class="col-12">';
				html += "<table><tr><td  width='40%' style='text-align: center;' class='font-workflow-content'> Name  :<span class='redmarker'>*</span></td><td width='60%' style='text-align: center;'><input class='form-control' name='ipstatusname' value='' maxlength='10'/></td></tr><tr><td width='40%' style='text-align: center;' class='font-workflow-content'><label>The Status</label></td><td width='60%' style='text-align: center;'><textarea class='form-control' id='txtstatus' value='' rows='10' cols='22'/></td></tr></table>"
				html +="</div></div>";
				div.append(html);
				div.css("z-index", "1");
				if(div.width() == 0) {
...
//					graph.refresh();
//				}
//				setTimeout(function() { $("input[name='ipstatusname']").focus( )}, 1000);
//				$("input[name='ipstatusname']").blur(function() { 
//					
//					if(this.value === '')
//						{
//							
//							Swal.fire({
//								title: 'Required value',
//								text:  "Please enter value for the status name",
//								type:  'error',
//								timer: 2000,
//								showConfirmButton: false
//							});
//							$(this).focus();
//							$('#isError', parent.document).val(1);
//							$('#messageError', parent.document).val('Please enter value for the status name');
//							
//						}
//					else if(this.value !== '')
//						{
...
//				});
			...
			// end process
			if(graph.view.getState(graph.getSelectionCell()).style.shape === 'triangle')
			{
				$('#approvedSearch').val('');
				...
				$.ajax({
					...
					success: function (data) {
						for(var n=0; n<data.length; n++) {
							...
							htmlct += '<td style="text-align: center;"><input class="groupApproval" type="checkbox" value=""/><span style="display:none">'+data[n]+'</span></td>';
				...
				$('#btnSearchUserApproved').unbind().on('click', function(e){
					var approved = $('#approvedSearch').val().trim();
					$.ajax({
						type: 'POST',
						url:  '/nera/api/get-all-user-approve',
						data: {approved: approved},
						success: function (data) {
							$('table#playbookApproveTable tbody').empty();
							var table = $('table#playbookApproveTable tbody');
							var htmlct = "";
							for(var n=0; n<data.length; n++) {
								htmlct += '<tr class="row-table-data">';
								htmlct += '<td style="text-align: center;"><input class="group' + i +'" type="checkbox" value=""/><span style="display:none">'+data[n]+'</span></td>';
								htmlct += "<td><label>"+data[n]+"</label></td>";
								htmlct += "</tr>";
							}
							table.append(htmlct);
				...
				$("#btnSelectApprove").unbind().on('click',function(e){
					var approval = doc.createElement('Approval');
//					approval.setAttribute('label', 'Approval');
					...
					html +="</div></div>";
					approval.setAttribute('label',approvedName);
					approval.setAttribute('approver',approver);
 ...
 * Creates the textfield for the given property.
 */
function createTextFields(graph, form, cell, attribute,tr,type)
{
	if(type == 'combo')
	{
		

		tr = form.addTexts(tr,input,attribute.nodeName+':', attribute.nodeValue);
	}
	else
	{
		var input = document.createElement('input');
